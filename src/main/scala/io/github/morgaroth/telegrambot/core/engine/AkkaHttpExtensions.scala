...
trait AkkaHttpExtensions extends MMarshalling {
  ...
  def handleUnsuccessfulResponse(req: HttpRequest)(resp: HttpResponse): HttpResponse = {
    resp.status match {
      ...
      case Conflict => throw WebhookConflict(resp)
  ...
  def request[R <: AnyRef : Manifest](req: HttpRequest, debug: Boolean = true)(implicit mat: Materializer, as: ActorSystem): Future[R] = {
    ...
    for {
      ...
      res <- Future.successful(rawRes).map(handleUnsuccessfulResponse(req))
