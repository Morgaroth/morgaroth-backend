...
class SessionRegistryActor(props: SocketIOSessionHandler) extends Actor with ActorLogging {
  ...
  def receive(actors: Map[UUID, ActorRef], clientsConnections: Map[UUID, ActorRef]): Receive = {

    case AskForSID(eio, sessionId) => //First connection
      val result: UUID = sessionId match {
        case Some(id) => id
        case None =>
          val sid = randomUUID
...
object SessionRegistryActor {
  ...
  def props(actor: SocketIOSessionHandler): Props = Props(new SessionRegistryActor(actor))
  ...
  case class AskForSID(eio: Int, sid: Option[UUID] = None)

  case class UpdateOut(sid: UUID, actor: ActorRef)

  case class Disconnect(sid: UUID)

  case class IncomingMessage(sid: UUID, message: String)
  ...
  private case class WrappedPing(sid: UUID, key: String) extends ActorRegistryCommunicationWrappers

  private case class WrappedPong(sid: UUID, key: String) extends ActorRegistryCommunicationWrappers

  private case class WrappedMessage(sid: UUID, text: String) extends ActorRegistryCommunicationWrappers

  private case class WrappedBroadcastOutgoingMessage(sid: UUID, text: String, excludeMe: Boolean = false) extends ActorRegistryCommunicationWrappers
